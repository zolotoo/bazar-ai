...
export async function transcribeWithGemini(apiKey, videoUrl) {
  ...
  const prompt = 'Расшифруй речь из этого аудио/видео. Верни ТОЛЬКО чистый транскрипт на языке оригинала, без тайм-кодов, без пояснений, без префиксов. Если речи нет (только музыка/тишина) — верни пустую строку.';

  // OpenRouter multimodal: official путь — input_audio (base64 + format).
  // mp4 это контейнер MPEG-4, по аудио-дорожке эквивалентен m4a — Gemini
  // демультиплексит контейнер сам. Пробуем форматы в порядке предпочтения.
  const formatsToTry = ['mp4', 'm4a'];
  let lastErr = null;
  for (const format of formatsToTry) {
    try {
      ...
      lastErr = e;
      console.warn(`[transcribeWithGemini] input_audio format=${format} failed: ${e.message}`);
    }
  }
  // Финальный fallback: file part (мало ли, где-то сработает)
  try {
    const { text } = await callOpenRouter({
      apiKey,
      model: MODELS.FLASH,
      messages: [
        {
          ...
          content: [
            ...
            {
              type: 'file',
              file: { filename: 'video.mp4', file_data: `data:video/mp4;base64,${base64}` },
            },
          ],
        },
      ],
