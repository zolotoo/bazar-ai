...
export async function transcribeWithGemini(apiKey, videoUrl) {
  ...
  const mimeType = resp.headers.get('content-type')?.split(';')[0].trim() || 'video/mp4';
  const base64 = buf.toString('base64');

  // Gemini через OpenRouter: видео отправляется как file part с data:video/mp4 URL.
  // Fallback на image_url если провайдер не примет file.
  const content = [
    {
      type: 'text',
      text: 'Расшифруй речь из этого видео. Верни ТОЛЬКО чистый транскрипт на языке оригинала, без тайм-кодов, без пояснений, без префиксов. Если речи нет (только музыка/тишина) — верни пустую строку.',
    },
    {
      type: 'file',
      file: {
  ...
  try {
    ...
    // Fallback: image_url wrapper (некоторые маршруты OpenRouter принимают видео так)
    console.warn('[transcribeWithGemini] file-part failed, retry via image_url:', e.message);
    const { text } = await callOpenRouter({
      apiKey,
      model: MODELS.FLASH,
      messages: [
