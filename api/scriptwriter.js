...
Верни ТОЛЬКО JSON, без пояснений, без markdown.`;
  ...
  // ── Шаг 2: Авто-улучшение — AI смотрит на оригинал + свой же JSON и исправляет ошибки ──
  try {
    const refinePrompt = `Ты только что проанализировал слайд карусели и создал этот JSON для редактора (холст 1080×1440px, координаты в пикселях):

${JSON.stringify({ elements: parsed.elements }, null, 2)}

Теперь посмотри на оригинальное изображение ЕЩЁ РАЗ и найди ошибки в JSON:
1. Координаты (x, y) — элементы стоят там где надо?
2. Размеры шрифтов — правильные?
3. Курсив (fontStyle) — все ли курсивные слова отмечены?
4. Пропущенные элементы — что забыл включить?
5. fontFamily — serif/sans-serif/heavy-sans/display/italic-serif правильно?
6. lineHeight и letterSpacing — соответствуют оригиналу?
7. zIndex — правильный порядок слоёв?
8. Смешанный стиль: если в строке часть текста курсив а часть нет — это отдельные элементы?

Верни ТОЛЬКО исправленный JSON. Если всё верно — верни тот же JSON. Без пояснений, без markdown:
{ "elements": [...] }`;

    const { text: refineText } = await callOpenRouter({
      apiKey: OPENROUTER_API_KEY,
    ...
    if (refineText) {
      const refined = parseJsonResponse(refineText);
      if (refined?.elements?.length > 0) {
        parsed.elements = refined.elements;
        console.log('Auto-refine: applied', refined.elements.length, 'elements');
      }
    }
  } catch (err) {
    console.error('Auto-refine error (non-fatal):', err.message);
    // Не фатально — продолжаем с первым проходом
  }

  // ── Шаг 3: ВСЕГДА генерируем фон через gemini-2.5-flash-image ───────────────────────────
