...
import { UserPlus, X, Shield, Edit, Eye, Trash2, Loader2, HelpCircle, LogOut, Send, Mail } from 'lucide-react';
...
export function ProjectMembersModal({ projectId, isOpen, onClose }: ProjectMembersModalProps) {
  ...
  const [inviteMode, setInviteMode] = useState<'telegram' | 'email'>('telegram');
  ...
  const handleInvite = async () => {
    ...
    if (!value) {
      toast.error(inviteMode === 'email' ? 'Введите email' : 'Введите Telegram username');
      return;
    }
    if (inviteMode === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value)) {
    ...
    setIsInviting(true);
    try {
      await inviteMember(value, selectedRole);
      if (inviteMode === 'email') {
        toast.success(`Приглашение отправлено на ${value}`);
      } else {
        toast.success(`Приглашение отправлено @${value.replace('@', '')}`);
      }
      setInviteValue('');
    } catch (error: any) {
      ...
      >
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto custom-scrollbar-light">
          ...
          <div className="mb-6 p-4 rounded-2xl bg-slate-50 border border-slate-100">
            <label className="block text-sm font-medium text-slate-700 mb-3 font-heading tracking-[-0.01em]">
              Пригласить участника
            </label>

            {/* Переключатель Telegram / Email */}
            <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-100 mb-3">
              <button
                onClick={() => { setInviteMode('telegram'); setInviteValue(''); }}
                className={cn(
                  "flex-1 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-medium transition-all touch-manipulation",
                  inviteMode === 'telegram'
              ...
              <button
                onClick={() => { setInviteMode('email'); setInviteValue(''); }}
                className={cn(
                  "flex-1 flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-medium transition-all touch-manipulation",
                  inviteMode === 'email'
                    ? "bg-white text-slate-800 shadow-sm"
            ...
            <div className="mb-3">
              <div className="relative">
                ...
                <input
                  ...
                  value={inviteValue}
                  onChange={(e) => {
                    const val = inviteMode === 'telegram' ? e.target.value.replace('@', '') : e.target.value;
                    setInviteValue(val);
                  }}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' && inviteValue.trim()) {
                      handleInvite();
                    }
                  }}
                  placeholder={inviteMode === 'email' ? 'user@example.com' : 'username'}
                  className={cn(
                    "w-full pr-4 py-3.5 min-h-[44px] rounded-2xl border border-slate-200/80 bg-white outline-none focus:ring-2 focus:ring-slate-200/50 focus:border-slate-400/50 transition-all text-slate-800 placeholder:text-slate-400 text-base touch-manipulation",
                    inviteMode === 'telegram' ? "pl-10" : "pl-4"
                  )}
                />
              </div>
            </div>
            ...
            <p className="text-xs text-slate-500 mt-3 px-1">
              {inviteMode === 'email'
                ? 'Пользователь получит письмо со ссылкой на проект'
                : 'Введите Telegram username без @ (например: username или @username)'}
            </p>
          </div>
            ...
            <div className="space-y-2">
              <AnimatePresence>
                {members.map((member: any) => {
                  const isEmailMember = member.user_id.startsWith('email-');
                  const displayName = isEmailMember
                    ? member.user_id.replace('email-', '')
                    : member.user_id.replace('tg-@', '').replace('tg-', '');
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold text-slate-800 truncate font-heading tracking-[-0.01em]">
                              {isEmailMember ? displayName : `@${username}`}
                            </span>
                            {isCurrentUser && (
