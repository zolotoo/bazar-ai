...
Верни ТОЛЬКО JSON, без пояснений, без markdown.`;
  ...
  // ── Шаг 2 + 3: параллельно — рефайн элементов И генерация фона ──────────────────────────
  const refinePromise = (async () => {
    const refinePrompt = `Ты только что проанализировал слайд карусели и создал этот JSON (холст 1080×1440px, координаты в пикселях):

${JSON.stringify({ elements: parsed.elements })}

Посмотри на изображение ЕЩЁ РАЗ и исправь ошибки: координаты, размеры шрифтов, курсив, пропущенные элементы, fontFamily, zIndex, смешанный стиль (курсив+обычный = отдельные элементы).
Верни ТОЛЬКО JSON без markdown: { "elements": [...] }`;
    ...
    if (!text) return;
    const refined = parseJsonResponse(text);
    if (refined?.elements?.length > 0) {
      parsed.elements = refined.elements;
      console.log('Auto-refine OK:', refined.elements.length, 'elements');
    }
  })().catch(err => console.error('Auto-refine error (non-fatal):', err.message));
  ...
  const bgPromise = (async () => {
    ...
    if (base64Chunks.length > 0) {
      ...
      console.warn('Gemini image gen: no image. Raw preview:', rawText.slice(0, 800));
    }
  })().catch(err => console.error('Gemini image gen error:', err.message));

  // Ждём оба параллельно
  await Promise.all([refinePromise, bgPromise]);
