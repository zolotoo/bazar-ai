...
1) Извлечь ХУК (первые 1–3 предложения из транскрипта, без воды — то, что удерживает зрителя в первые 3 секунды).
   ⚠️ ВАЖНО: хук ВСЕГДА возвращай на русском языке. Если транскрипт на английском/испанском/любом другом — ПЕРЕВЕДИ его на живой разговорный русский (не машинно, а как сказал бы носитель). Сохрани смысл и энергию, но язык — русский.
...
Дополнительно определи для каждой идеи:
- format: формат съёмки одним-двумя словами — "в камеру", "сравнение", "история", "разбор", "реакция", "эксперимент", "инструкция", "мнение", "список".
- difficulty: насколько сложно снять — "easy" (просто в камеру, без монтажа), "medium" (нужна b-roll или простой монтаж), "hard" (много сцен, локации, съёмка нужна подготовка).
- hook_pattern: какой приём в хуке — "вопрос", "провокация", "число", "обещание", "история", "контраст", "признание", "инструкция".
...
${userExamples || '—'}`;
  ...
  const { text } = await callOpenRouter({
    apiKey,
    // Flash вместо Pro: Pro тратит reasoning-токены, после них контент пустой при max_tokens~4500.
    // Flash дешевле ×20 и выдаёт 10 идей без проблем.
    model: MODELS.FLASH,
    messages: [{ role: 'user', content: prompt }],
    temperature: 0.8,
    max_tokens: 6000,
  ...
  if (!parsed || !Array.isArray(parsed?.ideas) || parsed.ideas.length === 0) {
    console.warn('[generateIdeas] empty or invalid result, raw:', (text || '').slice(0, 500));
  }
  return parsed;
}
